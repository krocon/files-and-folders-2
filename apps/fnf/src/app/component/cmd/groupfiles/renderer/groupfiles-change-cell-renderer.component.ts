...
export class GroupfilesChangeCellRendererComponent implements ComponentRendererIf<QueueFileOperationParams> {
  ...
  setData(
    ...
    if (!fileOperationParams?.target?.dir && !fileOperationParams?.target?.base) {
      this.text = '';

    } else if (
      fileOperationParams?.source?.dir !== fileOperationParams?.target?.dir
      || fileOperationParams?.source?.base !== fileOperationParams?.target?.base) {
      this.text = '→';
